...
app.post('/api/casino/play', async (req, res) => {
    const { userId, gameId, betAmount, prediction } = req.body;
    const bet = parseInt(betAmount);

    if (!userId || !gameId || isNaN(bet)) return res.status(400).json({ error: "Invalid data" });
    if (bet <= 0) return res.status(400).json({ error: "Bet must be positive" });

    // Validate Game
    const gameConfig = Object.values(CASINO_GAMES).find(g => g.id === gameId);
    if (!gameConfig) return res.status(400).json({ error: "Game not found" });
    if (bet < gameConfig.minBet) return res.status(400).json({ error: `Min bet is ${gameConfig.minBet}` });
    if (bet > gameConfig.maxBet) return res.status(400).json({ error: `Max bet is ${gameConfig.maxBet}` });
    ...
    try {
        ...
        // 1. Check Balance
        const [rows] = await connection.query('SELECT balance FROM staff_wallets WHERE discord_id = ? FOR UPDATE', [userId]);
        const currentBalance = rows.length > 0 ? parseInt(rows[0].balance) : 0;
        ...
        if (gameId === 'coinflip') {
            ...
            // Prediction: number 1-6. Win 5x (House edge 1/6 is 16.6%, payout 5x is generous but safe)
            // Or simple High/Low? Let's do simple Roll 1-100.
            // If user bets 'under 50' (1-49) or 'over 50' (51-100).
            // Let's stick to simple: Roll 1-100. If > 50 win 2x.
            // Actually user asked for "several modes". 
            // Let's do: User picks a range or specific number?
            // Let's implement "Lucky 7": Roll 2 dice.
            // Under 7 (x2), Over 7 (x2), Exact 7 (x5).
            
            const d1 = Math.floor(Math.random() * 6) + 1;
            const d2 = Math.floor(Math.random() * 6) + 1;
            const sum = d1 + d2;
            
            if (prediction === 'under' && sum < 7) { won = true; winAmount = bet * 2; }
            else if (prediction === 'over' && sum > 7) { won = true; winAmount = bet * 2; }
            else if (prediction === 'exact' && sum === 7) { won = true; winAmount = bet * 5; }
            
            resultData = { d1, d2, sum };
        }
            ...
            resultData = { reels: [r1, r2, r3] };

            if (r1 === r2 && r2 === r3) {
                won = true;
                if (r1 === '7️⃣') winAmount = bet * 10; // Jackpot
                else if (r1 === '💎') winAmount = bet * 5;
                else winAmount = bet * 3;
            } else if (r1 === r2 || r2 === r3 || r1 === r3) {
                won = true;
                winAmount = Math.floor(bet * 1.5); // Small win pair
            }
        }

        // 3. Update Balance
        // Deduct bet first (or net change)
        const netChange = winAmount - bet;
        const newBalance = currentBalance + netChange;

        await connection.query('UPDATE staff_wallets SET balance = ? WHERE discord_id = ?', [newBalance, userId]);

        // 4. Log Transaction
        const transType = won ? 'CASINO_WIN' : 'CASINO_LOSS';
        const comment = `Casino ${gameConfig.name}: ${won ? 'Won' : 'Lost'} (Bet: ${bet})`;
        
        await connection.query(
            'INSERT INTO staff_transactions (id, discord_id, type, amount, date, initiator, comment) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [crypto.randomUUID(), userId, transType, netChange, Date.now(), 'CASINO', comment]
        );

        await connection.commit();
        res.json({ success: true, won, winAmount, balance: newBalance, result: resultData });

    } catch (e) {
        await connection.rollback();
        console.error("Casino Error:", e);
        res.status(500).json({ error: "Casino processing error" });
    } finally {
        connection.release();
    }
});
